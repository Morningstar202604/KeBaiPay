...
async function shot(page, name){ await sleep(1500); await page.screenshot({ path: path.join(SHOT, name + '.png') }); console.log('📸', name) }
...
async function appFlow(app, base, loginCreds, views, shots){
  const browser = await chromium.launch({ headless: true, executablePath: process.env.PW_CHROMIUM, args:['--no-sandbox','--disable-dev-shm-usage'] })
  ...
  for (const [route, name] of views){ await page.goto(`${BASE}/${app==='h5'?'h5':app}/#/${route}`, { waitUntil:'networkidle' }); await sleep(2800); await shot(page, name) }
...
// === 商户后台 ===
{
  ...
  for (const [r,n] of [['dashboard','portal-dashboard'],['orders','portal-orders'],['reconciliation','portal-reconciliation'],['qrcodes','portal-qrcodes'],['apps','portal-apps'],['merchant','portal-merchant']]){ await p.goto(`${BASE}/portal/#/${r}`,{waitUntil:'networkidle'}); await sleep(2800); await shot(p,n) }
...
// === 用户 H5 ===
{
  ...
  for (const [r,n] of [['home','h5-home'],['agent','h5-agent'],['recharge','h5-recharge'],['redpacket','h5-redpacket'],['cashier','h5-cashier'],['bills','h5-bills']]){ await p.goto(`${BASE}/h5/#/${r}`,{waitUntil:'networkidle'}); await sleep(2800); await shot(p,n) }
  await p.close(); await ctx.close()
}
// === 管理后台 ===
{
  ...
  for (const [r,n] of [['dashboard','admin-dashboard'],['users','admin-users'],['merchants','admin-merchants'],['withdrawals','admin-withdrawals'],['orders','admin-orders'],['finance','admin-finance'],['risk','admin-risk'],['agents','admin-agents']]){ await p.goto(`${BASE}/admin/#/${r}`,{waitUntil:'networkidle'}); await sleep(2800); await shot(p,n) }
  await p.close(); await ctx.close()
}
await browser.close()
// 重命名视频
