...
> 现状说明：本项目源自路线图「P0 商户后台前端 SPA」。当前实现 **数据看板 / 订单管理 / 对账查询 / 收款码 / 应用管理 / 商户资料** 六个模块，调用后端真实接口（`/merchants/*`、`/cashier/orders/*`、`/auth/login`）。
...
│   └── views/               # LoginView / DashboardView / OrdersView / ReconciliationView / QrCodesView / AppsView / MerchantInfoView
```
...
- [ ] 商户入驻申请流程页（register 引导）
- [ ] 用户端 H5（钱包 / 收银台 / 红包）Vue 3 化（路线图 P1）
- [ ] Webhook 回调重试可视化（路线图 P1）
- [ ] 管理后台 Vue 3 化
