...
**发布日期：** 2026-08-01
**版本类型：** 功能增强（官方渠道 SDK 接入 + Connector 体系接入业务链路）

| 维度 | 数量 | 说明 |
|---|---|---|
| API 端点 | 214 个 | 较 v1.0 的 ~80 增长 167% |
| Prisma 模型 | 52 个 | 较 v1.0 的 ~20 增长 160%，按 15 个业务域分组 |
| 单元测试 | 1156+ 个（随修复持续增长） | 73+ 套件（2026-08-26 实测口径；数字以 `npx jest --listTests` 与最近一次 CI 为准） |
| E2E 测试 | 48 个用例 / 5 个套件 | 含 agent(33) + user-scenarios(7) 等；2026-08-26 勘误：此前文档曾误记为 324 个 |
| 模块数 | 42 个 | NestJS 模块 |
| 文档数 | 15 个 | README + docs/ 下 14 篇 |
...
timeline
    ...
    2026-08-01 : v2.2 官方 SDK + Connector 桥接
              : 支付宝 alipay-sdk / 微信 wechatpay-node-v3 替换手写签名
              : ConnectorRegistry 自动注册 / PaymentChannelBridge 接入业务链路
              : 充值/代付/退款/查询经 ConnectorRouter 统一重试
```
...
| v2.2 | 2026-08-01 | 官方渠道 SDK（支付宝/微信）替换手写签名 + Connector 体系接入业务链路（ConnectorRouter 桥接） |
...
| 测试覆盖 | 单测 1156+ / E2E 48（勘误后实测口径），关键路径含 `concurrency.spec.ts` 并发测试 |
...
| 序号 | 任务 | 优先级 | 说明 | 状态 |
|---|---|---|---|---|
| 1 | 真实微信支付 v3 接入 | P0 | 替换当前 mock，接入微信商家转账到零钱 v3、JSAPI、Native、退款 | 已接入 SDK（v2.2） |
| 2 | 真实支付宝 v3 接入 | P0 | 接入支付宝当面付、电脑网站支付、批量付款到账户 | 已接入 SDK（v2.2） |
| 3 | 商户后台前端 SPA（Vue 3） | P0 | Vue 3 + Pinia + Element Plus，含订单/对账/风控面板 | ✅ 已落地（`web/` → `/portal`） |
| 4 | 用户端 H5 优化（Vue 3 + Vite） | P1 | 收银台 / 钱包 / 红包模块用 Vue 3 重写，移动端体验优化 | ✅ 已落地（`web-h5/` → `/h5`） |
| 5 | Webhook 重试可视化 | P1 | 管理后台展示回调重试队列、手动重发、状态机可视化 | ✅ 已落地（商户后台订单回调重发） |
| 6 | 管理后台 Vue 3 SPA | P1 | Vue 3 重写管理后台，含用户/商户/提现/订单/财务/风控面板 | ✅ 已落地（`web-admin/` → `/admin`） |
