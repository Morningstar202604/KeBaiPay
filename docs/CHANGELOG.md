...
- [版本 0.2.2（2026-09-06）](#版本-0222026-09-06)
...
### 【高】全端日期序列化 + 路由过渡稳定性修复（v0.2.2 续）

- **【高】全局响应拦截器洗掉 Date 字段**：stripSensitiveFields 递归重建响应对象时用 Object.entries，Date 实例无可枚举属性 → **全端所有接口的日期字段均被序列化为 {}**（用户账单时间、后台注册时间等全部损坏，前端字符串方法直接抛错）。修复：Date 实例原样透传，JSON 序列化时自然输出 ISO 字符串。
...
- **三端前端包体瘦身（Element Plus 按需引入）**：全量 `use(ElementPlus)` + 全量 CSS 改为 `unplugin-vue-components` + `ElementPlusResolver` 按需注册模板组件；`ElMessage`/`ElMessageBox` 等函数式组件是显式 import，样式需手动补齐；中文 locale 由 `el-config-provider` 继续提供。实测主包（gzip）：
  - web-h5：366KB → **90KB**（-75%）
  - web-admin：1.07MB → **92KB**（-91%）
  - web（商户门户）：1.1MB → **91KB**（-92%）
- **echarts 按需注册**：商户门户 Dashboard 从 `import * as echarts from 'echarts'` 全量引入改为 `echarts/core` + `BarChart` + Grid/Tooltip/Legend 组件 + CanvasRenderer 按需注册，Dashboard 懒加载 chunk 1.13MB → 511KB（gzip 175KB）。
- 验证：三端 Playwright 冒烟通过（登录跳转、ElMessage 弹出、Dashboard 图表 canvas 真实渲染、订单页表格），全程无 JS 错误、无 CSP 违规。

### 安全（纵深防御补强）

- **自由文本字段输入侧收紧**：新增 `@IsSafeText` 自定义校验装饰器（拒绝 `<`、`>` 与零宽控制字符），红包备注、转账留言、商户名等 37 个自由文本 DTO 字段换装。与 v0.3.1 的输出转义形成双层防线：即使未来某处输出转义遗漏，恶意标记也无法进入存储。恶意请求返回 400 与明确中文提示。
- 覆盖率门禁校准为真实基线（此前 80% 门禁从未真正生效），防止门禁长期失守形同虚设。

### 测试

- **红包服务补测 31 用例**：二倍均值法边界（最后一人收敛、每人 1 分、不超发）、四种红包类型创建约束、领取状态机（过期/领完/自领/指定人不符/口令错误）、幂等重放、乐观锁抢空、过期退差额等核心资金路径。
- **担保交易补测 11 用例**：退款申请/同意/拒绝全链路复式分录、冻结余额守卫、并发抢空回滚、自动收货调度容错。
...
- **修复原生 SPA 的存储型 XSS（高危）**：商户名、转账备注、账单对方户名等用户可控字段经模板字符串直插 `innerHTML`，攻击者可在商户入驻资料中注入 `<img onerror>` 之类 payload，管理员打开审核页即触发，可读取 `localStorage` 中的管理端令牌。处理：
  - `escapeHtml()` 改为字符表替换，覆盖 `& < > " '`（原实现不转义引号，放进 HTML 属性仍可逃逸；顺带修复 `!str` 判空吞掉 `0`/`false` 的问题）；
  - 新增 `jsStr()` / `jsNum()` / `jsBool()`，34 处内联 `onclick` 的字符串参数改经 `JSON.stringify` + HTML 转义双层处理，数字与布尔参数强制收敛类型；
  - 全部 195 个 HTML 模板块中的动态插值补齐转义，`escapeHtml` 调用从 11 处增至 328 处；URL 拼接、`Authorization` 头等非 HTML 模板刻意保持不转义，避免 `&` 被实体化破坏路由。
  - 验证：8 组典型 payload 在文本/属性/内联事件三种上下文下往返一致；真实接口写入恶意商户名后，Chromium 打开审核页确认 payload 不执行、以纯文本呈现。

- **修复 CSP 拦截内联事件导致的按钮失效（功能性回退）**：v0.3.0 的 `script-src-attr 'none'` 会静默拦截全部内联 `onclick`，管理端与商户端的操作按钮实际已无法点击。现调整为 `'unsafe-inline'`——内联事件的注入风险已在源头由 `jsStr`/`jsNum` 封死，`script-src 'self'` 仍阻止外部脚本与内联 `<script>`；待原生 SPA 退役（入口收敛到 Vue 三端）后改回 `'none'`。

- **`/metrics` 生产环境强制鉴权**：`NODE_ENV=production` 下未配置 `METRICS_TOKEN` 时拒绝启动（`SecurityValidatorService` 新增校验，token 长度不少于 24 位），并在控制器层兜底返回 401，防止绕过启动校验的部署方式。

### 文档

- 新增 `docs/SECURITY_AUDIT.md` 全面排查报告，含本次 XSS 的完整可利用链与修复记录。

---

## 版本 0.3.0（2026-09-14）

**版本类型：** 新功能（Phase 1 可用性——打通商户自助开通漏斗）+ 许可证变更
...
- **仓库可发现性配置**：`robots.txt` / `sitemap.xml` / social preview 图 / PWA manifest，三端页面补齐 description、keywords 与 Open Graph 卡片。代码写得再好，搜不到等于不存在——这部分和写功能一样是发布的一部分。

### 安全

- **依赖漏洞清零（9 个，其中 7 个 high）**
  - `multer` 2.2.0 → 2.3.0：修 4 个 CVE（构造 multipart 字段名触发 DoS、上传中止时文件描述符泄漏、异步 `fileFilter` 竞态绕过大小限制、超大数组索引 DoS）。
  - `nodemailer` 9.0.6 → 9.1.1：修 4 个 CVE（`resolveContent()` 旧签名绕过 `disableFileAccess`/`disableUrlAccess`、IDN/Punycode 域名白名单绕过、addressparser O(n²) 复杂度 DoS、RFC 5322 注释解析导致收件域校验绕过）。
  - `mailparser` 内嵌的旧 nodemailer 通过嵌套 `overrides` 一并提升，避免同一 CVE 从依赖链二次引入。
  - 备注：`@nestjs/core`、`platform-express`、`schedule`、`serve-static`、`swagger` 五个包报的 high 全部是 multer 的传递依赖误报，升 multer 后自动清零，**未改动框架版本**。

### 变更

- **许可证由 PolyForm Noncommercial 1.0.0 改为 MIT。** NC 条款挡住的是"学习之外的一切用途"，而这套代码最有价值的用法恰恰是被集成进真实业务里跑；同时 NC 与部分依赖的许可组合存在兼容争议，对使用者是额外的法务负担。改 MIT 后商业集成、二次分发、SaaS 化均无需授权。
  - ⚠️ 许可证不豁免运营责任：含平台内账本的支付业务仍需持牌或走服务商模式，见 [合规分析](EXPERT_PANEL_ASSESSMENT.md)。

### 验证

- 三前端 vue-tsc 构建全绿（h5 / portal / admin）
- 后端 tsc EXIT=0；users/merchants 套件 63 用例通过（沙箱关闭路径行为不变）
- 全量单测 77 套件 / 1178 用例通过；`npm audit --omit=dev` 报 0 vulnerabilities
