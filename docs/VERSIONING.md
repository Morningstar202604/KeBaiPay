...
| 位 | 何时递增 | 典型场景 | 示例 |
|---|---|---|---|
| **PATCH（第三位）** | 纯修复，不新增用户可见能力 | Bug 修复、安全修复、性能优化、文档修正、依赖小升级 | 修复回调金额校验漏洞 → `2.2.0 → 2.2.1` |
| **MINOR（第二位）** | 向后兼容的新功能 | 新业务模块、新 API 端点、新前端页面、新渠道连接器、非破坏性 schema 迁移（只加表/加列） | 上线订阅产品化页面 → `2.2.x → 2.3.0` |
| **MAJOR（第一位）** | 破坏性变更 | API 契约不兼容改动、删除/重命名端点、需要数据迁移或停机窗口的 schema 变更、配置项不兼容、最低 Node/PG 版本提升 | 影子账本改造（账本语义变更）→ `2.x → 3.0.0` |
...
1. **日常开发默认 PATCH。** 连续多个修复各自发 `2.2.1 / 2.2.2 / 2.2.3…`，
   禁止把一批修复打包成 `2.3.0` 冲高次版本号。
2. MINOR 必须有对应 CHANGELOG 的"新增"条目支撑；凑不出新功能条目就不许动第二位。
3. MAJOR 必须附迁移指南（`docs/` 下新增 `MIGRATION-vX.md`）。
4. 预发布版本用 SemVer 后缀：`2.3.0-beta.1`、`3.0.0-rc.1`。
...
## 6. 历史遗留对齐说明（2026-08-26）

评审发现三处版本漂移：根 `2.2.0` / 三前端 `2.3.0` / CHANGELOG `2.2.1（进行中）`。
本次统一收敛：全部对齐为 **2.2.1**（当前进行中的修复批次），前端虚高的 `2.3.0` 回退。
此后以本文档规则执行。
