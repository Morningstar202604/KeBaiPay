...
>
> **v0.3.2 追踪**：报告指出的问题已在陆续兑现——覆盖率阈值调至真实基线并从 v0.3.2 起在 CI 强制执行（`npm run test:cov`）；红包（+31 用例）、担保退款（+11 用例）、security（+12 用例）已补齐，全量 1244 用例 / 80 套件，覆盖率 55.6%/50.7%/53.2%/56.5%。本报告正文保留 v0.2.2 时点的原始快照，不回写。
